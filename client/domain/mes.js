class Mes {
    constructor(nome) {
        if (nome === '') throw new Error('Mês inválido: o mês é obrigatório')
        this.nome = nome
        this.saldoInicial = 0
        this.totalizador = {saldo: 0, juros: 0, rendimentos: 0, receita: 0, despesa: 0, distribuicaoDeDespesas: []}
        this.lancamentos = []
    }
    arredondar(valor){
    ...
    apurarDespesas() {
        for(const lancamento of this.lancamentos) {
            if (lancamento.tipo === 'despesa'){
                this.totalizador.saldo -= lancamento.valor
                this.totalizador.despesa += lancamento.valor
            }
        }
    }
    distribuirDespesas () {
        ...
        for(const lancamento of this.lancamentos) {
            if (lancamento.tipo === 'despesa') {
                    const percentual = this.arredondar((lancamento.valor/this.totalizador.despesa)*100)
